class Api::V1::PropositionsController < Api::V1::ApplicationController
  ...
  # POST OR DELETE /api/v1/propositions/1/against
  def against
    if request.post?
      user_action = UserAction.find_or_create_by(user_id: current_user.id, proposition_id: @proposition.id, action: 'against')
      head :ok
    elsif request.delete?
      user_action = UserAction.find_by(user_id: current_user.id, proposition_id: @proposition.id, action: 'against')
      user_action.try :destroy
      head :ok
    else
      render text: {errors: "Wrong HTTP method"}.to_json, status: :unprocessable_entity, layout: 'api_v1'
  ...
  # POST OR DELETE /api/v1/propositions/1/favorite
  def favorite
    if request.post?
      user_action = UserAction.find_or_create_by(user_id: current_user.id, proposition_id: @proposition.id, action: 'favorite')
      head :ok
    elsif request.delete?
      user_action = UserAction.find_by(user_id: current_user.id, proposition_id: @proposition.id, action: 'favorite')
      user_action.try :destroy
      head :ok
    else
      render text: {errors: "Wrong HTTP method"}.to_json, status: :unprocessable_entity, layout: 'api_v1'
